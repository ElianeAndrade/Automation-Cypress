...
const origin = ambiente === 'stg'
  ? 'https://staging-id-pf-sts.Nome_Instituição.com.br'
  : 'https://id-pf-sts.Nome_Instituição.com.br';


class HomeNome_InstituiçãoPage {
  buscarVaga() {
  ...
  botaoQueroUsarPotencialNome_Instituição() {
    cy.get('a[title="Quero usar todo o potencial do Nome_Instituição"]').click();
    cy.wait(5000);
  ...
  cadastrarCurriculoGratis() {
    cy.get('a[class="btn-purple-hover-effect button-step-Nome_Instituição-track"]').click({ force: true });
    cy.get('div[class="container backstage__item identity__card__body"]', { timeout: 10000 }).should('be.visible');
  }
}

export default new HomeNome_InstituiçãoPage();